...
def visualize_3pcd(pdata):
    color_list = [[0,0,1],[0,0,0],[1, 0.9, 0]]
    pcd_list = []
...
lidar_loc = np.zeros((50,3))
lidar_loc[0:50,2] = np.arange(1,51)/20

def integrate_point_cloud2(data_list, transform_list):
    xyz_full = np.zeros((0,3))
    xyz_sensor = np.zeros((0,3))
...
#        print("\n dx {}, dy {}, dz {}".format(dx, dy, dz))
        ...
        sensor_world = world_transform(lidar_loc, dx, dy, dz, yaw, pitch, roll)
        xyz_sensor = np.vstack((xyz_sensor,sensor_world))
    # xyz_full = np.vstack((xyz_full,w_axis))
    return [xyz_full, xyz_sensor]
#                pcl_list = [d1, d2, d3]      
multi_lidar_res, sensor_res = integrate_point_cloud2(data, lvp_list) # integrated 3-lidar point cloud in one single frame
...
visualize_3pcd([ground, non_ground])